...
![image](https://github.com/user-attachments/assets/0fde014b-1448-4cf4-adde-c1944496a842)
...
![greedy](https://github.com/user-attachments/assets/206ba3ce-3d9f-4453-8e73-5ff53d6b99f6)
                                    **Greedy Search (Tìm kiếm tham lam)**
                                    *(Luôn chọn nút có $h(n)$ nhỏ nhất)*

![A_star](https://github.com/user-attachments/assets/6b497924-dd23-4c81-8fa4-6bbd3b9ce40e)
                                    **A\* Search (A-star)**
                                    *(Chọn nút có $f(n) = g(n) + h(n)$ nhỏ nhất)*
  
![IDA](https://github.com/user-attachments/assets/4d135861-db2a-49fa-8e65-033a84c186e4)
...
![image](https://github.com/user-attachments/assets/1d1a5a09-a816-4126-9f7b-bfa190633328)
...
* **Hill Climbing (Simple, Steepest, Stochastic):** Tưởng tượng một agent "leo" lên một "ngọn đồi" trong không gian trạng thái, luôn cố gắng di chuyển đến trạng thái lân cận "cao hơn" (tốt hơn). Tìm "Hill climbing algorithm gif".
* **Simulated Annealing (SA):** Giống Hill Climbing, nhưng thỉnh thoảng chấp nhận di chuyển đến trạng thái "thấp hơn" (xấu hơn) để có cơ hội thoát khỏi các đỉnh cục bộ và tìm đỉnh cao hơn. Tìm "Simulated annealing gif".
* **Genetic Algorithms (GA):** Hình dung một "quần thể" các giải pháp tiềm năng "tiến hóa" qua các "thế hệ" thông qua "lai ghép" và "đột biến". Tìm "Genetic algorithm visualization gif".
* **Beam Search:** Giữ lại một số lượng giới hạn (beam width) các trạng thái "hứa hẹn" nhất ở mỗi bước và chỉ khám phá từ những trạng thái đó. Tìm "Beam search algorithm gif".
...
![image](https://github.com/user-attachments/assets/aed3b324-8868-428f-8564-c0d474faf85d)
*Ghi chú về hiệu suất thực tế trên 8-puzzle:*
...
* **AND-OR Tree Search:** Hình dung một bài toán lớn được chia thành các phần nhỏ hơn. Để giải bài toán lớn, bạn cần giải tất cả các phần "AND" hoặc một trong các phần "OR". Tìm "AND OR tree search visualization".
* **Partially Observable:** Tưởng tượng agent di chuyển trong sương mù, chỉ thấy được một phần nhỏ xung quanh. Agent phải suy luận về vị trí có thể của mình. Tìm "POMDP visualization" hoặc "belief state update gif".
* **Unknown/Dynamic Environment:** Agent khám phá một mê cung chưa biết bản đồ, hoặc các bức tường trong mê cung thay đổi vị trí. Tìm "robot exploration unknown environment gif".
...
*(Phần này được giữ nguyên theo yêu cầu vì chưa có code tương ứng)*
...
* **Các thuật toán chính:** Backtracking Search, Forward Checking, AC-3 (Arc Consistency Algorithm 3).
...
* **Backtracking Search:** Tưởng tượng việc thử gán giá trị cho từng biến, nếu một gán giá trị dẫn đến vi phạm ràng buộc thì "quay lui" và thử giá trị khác. Tìm "Backtracking algorithm gif" hoặc "N-Queens backtracking gif".
* **Forward Checking / AC-3:** Khó có GIF riêng biệt, nhưng chúng là các kỹ thuật giúp tăng tốc Backtracking bằng cách loại bỏ sớm các giá trị không thể nằm trong giải pháp từ miền giá trị của các biến khác. Tìm "Constraint propagation animation".
